...
import { celebrateConfetti, playBeepSimple } from "../utils/reward";
...
export default function PomodoroTimer({ tasks }: PomodoroTimerProps) {
	...
	function playBeep() {
		...
		playBeepSimple(880, 0.3, "sine", 0.1);
	}

	async function notifySessionEnd(currentMode: "focus" | "break", wasLongBreak: boolean) {
		playBeep();
		// Small celebration when completing a focus session
		if (currentMode === "focus") {
			celebrateConfetti({ withBeep: false, particleCount: 70, spread: 60, startVelocity: 35, origin: { y: 0.65 } });
		}
		...
		<div className="card p-4 animate-fade-in">
			...
			<div className="grid md:grid-cols-[1fr_auto_auto] gap-3 items-center">
				...
				<div className="flex items-center gap-2">
					<label className="text-sm text-ink-500">Task</label>
					<select
						className="input md:w-[220px]"
